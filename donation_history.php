...
<div class="container d-flex align-items-center justify-content-center color-bg text-center" id="whoweare">
    <div> 
        <h4><?php echo $_SESSION['user_first_name'] ?>'s donation history</h4>
        <br>
        <br>
        <div class="container">
      <p>Your donation activity log: (From oldest to newest): </p>
      <br>
      <table class="table table-striped table-primary" >
          ...
          <tbody>
            ...
            <?php
              $ID= $_SESSION['user_id'];
							$sql= "SELECT receiver_name,amount,account FROM donation_details_1 WHERE donor_id='$ID'";
							$res=mysqli_query($conn, $sql);
							
							while ($row= mysqli_fetch_assoc($res)){
			
								echo "<tr><td>{$row["receiver_name"]}</td><td>{$row["amount"]}</td><td>{$row["account"]}</td></tr>";
							}
